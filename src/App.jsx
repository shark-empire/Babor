...
import { CheckCircle, AlertCircle, Loader2, ShoppingCart, Plus, Minus } from 'lucide-react';
...
export default function App() {
  ...
  const [quantity, setQuantity] = useState(1); // 1. Added quantity state
  
  // States: 'idle' | 'verifying' | 'success' | 'error'
  const [status, setStatus] = useState('idle'); 
  const [purchasedPins, setPurchasedPins] = useState([]); // 2. Changed to an array to hold multiple PINs
  ...
  const paystackConfig = {
    ...
    email: email,
    amount: totalPrice * 100, // 3. Multiplied by quantity (converted to pesewas)
    publicKey: PAYSTACK_PUBLIC_KEY,
    metadata: {
      phone: phone,
      product_id: selectedProduct.id,
      product_name: selectedProduct.name,
      quantity: quantity, // 4. Included quantity in Paystack metadata dashboard
    },
  };
  ...
  const verifyPaymentWithBackend = async (reference) => {
    setStatus('verifying');
    try {
      // 5. Send both reference and quantity to the server
      ...
      // 6. Expecting an array of pins from your backend now: e.g., [{pin: '...', serial: '...'}, ...]
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center p-4">
        ...
        <p className="text-gray-500 mt-2 text-center max-w-md">
          Please wait while we generate your secure {quantity}x {selectedProduct.name} PIN(s). Do not close this page.
        </p>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center p-4">
        <div className="bg-white p-6 rounded-xl shadow-lg max-w-md w-full text-center border-t-4 border-green-500">
          ...
          {/* 7. Loop through and display all purchased PINs */}
    ...
    <div className="min-h-screen bg-gray-100 flex items-center justify-center p-4">
      <div className="bg-white max-w-lg w-full rounded-2xl shadow-xl overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          {/* 8. Quantity Selector UI Element */}
          <div>
            <label className="block text-sm font-semibold text-gray-700 mb-2">Quantity (Bulk Purchase)</label>
